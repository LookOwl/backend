...
from old.api.dtos.login_dto import LoginDto , RegisterUserDto
from old.services.auth_service import AuthService, UserAlreadyExistsException, UnknownException
from old.dependencies.services import get_auth_service
...
async def register(registerDto : RegisterUserDto, authService : AuthService = Depends(get_auth_service)):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="No se pudo crear el usuario. Error: {}".format(str(e))
        )
